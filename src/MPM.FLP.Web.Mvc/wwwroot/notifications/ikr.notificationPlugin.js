    ...
    $.fn.ikrNotificationSetup = function (options) {
       ...
       */
        ...
        if ($.trim(parentId) !== "" && parentId.length > 0) {
            $("#" + parentId).append("<div id='notif_counter' class='ikrNoti_Counter'></div>" +
                "<div id='notif_button' class='ikrNoti_Button'></div>" +
        ...
        if ($.trim(parentId) !== "" && parentId.length > 0) {
            ...
            if (defaultSettings.NotificationList.length > 0) {
                $.map(defaultSettings.NotificationList, function (item) {
                    ...
                    var creationTime = date.getDate() + "-" + m + "-" + date.getFullYear() + " " + date.getHours() + ":" + date.getMinutes();
                    var readed = "";
                    if (!item.isRead) {
                        readed = "New";
                    }
                    ...
                    $("#" + parentId + " .ikrNotificationItems").append("<div class='ikrSingleNotiDiv" + className + "' notiId=" + item.id + ">" +
                        "<h4 class='ikrNotiFromPropName'>" + readed + "</h4>" +
                        ...
                        "<div class='ikrNofiCreatedDate'>" + creationTime + "</div>" +
                        "</div>");
                    $("#" + parentId + " .ikrNotificationItems .ikrSingleNotiDiv[notiId=" + item.id + "]").click(function () {
                        if ($.trim(item.url) !== "") {
                            $.ajax({
                                type: "GET",
                                url: '/Notifications/SetRead?notiId=' + item.id,
                                traditional: true,
                                dataType: "json",
