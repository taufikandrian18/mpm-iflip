...
{
    [AbpMvcAuthorize]
    public class KategoriApparelController : FLPControllerBase
    {
        private readonly ApparelCategoryAppService _appService;

        public KategoriApparelController(ApparelCategoryAppService appService)
        {
        ...
        //    return View(model);
        //}
        public IActionResult Create(ApparelCategories model)
        {
            return View(model);
        }

        [HttpPost]
        public async Task<IActionResult> Create(ApparelCategories model, string submit, IEnumerable<IFormFile> files)
        {
            if (model != null)
            {
                if (model.Name == null)
                ...
                if (files.Count() > 0)
                {
                    ...
                    model.IconUrl = await azureController.InsertAndGetUrlAzure(files.FirstOrDefault(), model.Id.ToString(), "IMG", "kategoriapparel");
                }
        ...
        private async Task<string> InsertToAzure(IFormFile file, ApparelCategories model)
        {
            string url = "";
            ...
            if (CloudStorageAccount.TryParse(conn, out cloudStorage))
            {
                ...
                CloudBlobContainer cloudBlobContainer = cloudBlobClient.GetContainerReference("kategoriapparel");
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(ApparelCategories model, string submit, IEnumerable<IFormFile> files)
        {
            if (model != null)
            {
                if (model.Name == null)
