...
{
    [AbpMvcAuthorize]
    public class ServiceProgramManagementController : FLPControllerBase
    {
        ...
        //    return View(model);
        //}
        public IActionResult Create(ServicePrograms model)
        {
            return View(model);
        }

        [HttpPost]
        public async Task<IActionResult> Create(ServicePrograms model, string submit, IEnumerable<IFormFile> files, IEnumerable<IFormFile> images, IEnumerable<IFormFile> videos)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(ServicePrograms model, string submit)
        {
            if (model != null)
            {
                if (model.Title == null)
        ...
        public IActionResult UploadAttachment([DataSourceRequest]DataSourceRequest request, string submit, Guid Id, IEnumerable<IFormFile> images, IEnumerable<IFormFile> videos, IEnumerable<IFormFile> documents)
        {
            ...
            if (model != null)
            {
                if (files.Count() > 0)
                {
                    var tmp = _appService.GetById(Id).ServiceProgramAttachments.Where(x => x.Title.Contains("IMG") && string.IsNullOrEmpty(x.DeleterUsername)).Count();
                    if (images.Count() > 10)
                    {
                        TempData["alert"] = "Gambar yang dimasukkan tidak dapat lebih dari 10";
                        TempData["success"] = "";
        ...
        public async Task<IActionResult> Grid_Read([DataSourceRequest]DataSourceRequest request)
        {
            ...
            var activities = await _activityLogAppService.GetContentActivityLogsSummary(null, null, "serviceprogram");
        ...
        public IActionResult GridAttachment_Destroy([DataSourceRequest]DataSourceRequest request, Guid modelId, ServiceProgramAttachments item)
        {
            if (ModelState.IsValid)
            {
                ...
                if (!string.IsNullOrEmpty(model.FeaturedImageUrl))
                {
                    ...
                    {
                        var attachments = model.ServiceProgramAttachments.Where(y => string.IsNullOrEmpty(y.DeleterUsername));
