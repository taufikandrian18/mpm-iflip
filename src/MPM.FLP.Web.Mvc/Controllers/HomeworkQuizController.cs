...
{
    [AbpMvcAuthorize]
    public class HomeworkQuizController : FLPControllerBase
    {
        ...
        public IActionResult Create(HomeworkQuizzes model)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(HomeworkQuizzes model, string submit, IEnumerable<IFormFile> images)
        {
            if (model != null)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(HomeworkQuizzes model, string submit, IEnumerable<IFormFile> images)
        {
            if (model != null)
            {
                model.LastModifierUsername = this.User.Identity.Name;
        ...
        //Homework Question
        public IActionResult HomeworkQuestion(Guid id)
        {
            var item = _appService.GetById(id);
            return View(item);
        }
        public IActionResult EditQuestion(Guid id)
        {
            ...
            model.ParentId = item.HomeworkQuizId;
        ...
        {
            HomeworkQuizQuestions model = new HomeworkQuizQuestions();
            if (model != null)
            {
                model.Id = Guid.NewGuid();
                model.CreationTime = DateTime.UtcNow.AddHours(7);
                ...
                model.OptionE = item.OptionE;
                model.CorrectOption = item.CorrectOption;
                model.HomeworkQuizId = item.ParentId;
        ...
        {
            HomeworkQuizQuestions model = _questionAppService.GetById(item.Id);
            if (model != null)
            {
                model.LastModifierUsername = this.User.Identity.Name;
                model.LastModificationTime = DateTime.UtcNow.AddHours(7);
        ...
        public IActionResult GridQuestion_Read([DataSourceRequest]DataSourceRequest request, HomeworkQuizzes model)
        {
            ...
            DataSourceResult result = _questionAppService.GetAll().Where(x => string.IsNullOrEmpty(x.DeleterUsername) && x.HomeworkQuizId == model.Id).OrderBy(x => x.CreationTime).ToDataSourceResult(request);
        ...
        [HttpPost]
        public async Task<IActionResult> ImportExcel(HomeworkQuizzes model, string submit, IEnumerable<IFormFile> files)
        {
            if (model != null)
            {
                if (files.Count() > 0)
                {
                    ...
                    if (!Path.GetExtension(file.FileName).Equals(".xlsx", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        return Redirect("~/HomeworkQuiz/ImportExcel/" + model.Id);
                    }
            ...
            return Redirect("~/HomeworkQuiz/HomeworkQuestion/" + model.Id);
        }
        ...
        public async Task<IActionResult> GetCountAssignedUser(Guid id)
        {
            ...
            var assignedUser = _assignmentAppService.GetAll().Where(x => x.HomeworkQuizId == id).Select(x => x.IDMPM).ToList();
        ...
        public IActionResult Cascading_Get_Dealer([DataSourceRequest]DataSourceRequest request, string kota)
        {
            ...
            if (kota.IsNullOrWhiteSpace() || kota == " Semua Kota")
            {
                ...
                var dealer = _dealerAppService.GetAll().Where(x=> x.Kota != "").Select(x => new { kota = x.Kota, dealer = x.Nama, kode = x.KodeDealerMPM }).Distinct().ToList();
                result = dealer.OrderBy(x => x.kode).ThenBy(x => x.dealer).ToDataSourceResult(req);
                 return Json(result);
            ...
            else
            {
                ...
                result = dealer.OrderBy(x => x.kode).ThenBy(x => x.dealer).ToDataSourceResult(request);
                return Json(result);
        ...
        public async Task<IActionResult> Get_User([DataSourceRequest]DataSourceRequest request, string id,  string channel, string karesidenan, string kota, string dealer, string jabatan, int skip, int pageSize)
        {
            ...
            //var data = _internalUserAppService.GetAll().Skip(skip).Take(pageSize).ToList();
            //var total = _internalUserAppService.GetAll().Count();
            ...
            //return Json(internalUser.ToDataSourceResult(request));
            //return Json(task.OrderBy(x => x.IDGroupJabatan).ThenBy(x => x.IDMPM));
        ...
        public async Task<IActionResult> Get_Users([DataSourceRequest] DataSourceRequest request, string id, string channel, string karesidenan, string kota, string dealer, string jabatan, int limit, int page)
        {
            ...
            //return Json(internalUser.ToDataSourceResult(request));
            //return Json(task.OrderBy(x => x.IDGroupJabatan).ThenBy(x => x.IDMPM));
            var data = task.Take(limit).Skip(page).ToList().OrderBy(x => x.IDGroupJabatan).ThenBy(x => x.IDMPM);
            return Json(new { 
            data = data,
            count=count
            });
        ...
        public IActionResult HwQuizScore(Guid id)
        {
            var item = _appService.GetById(id);
            return View(item);
        }

        public IActionResult Grid_Read_HwQuizScore([DataSourceRequest]DataSourceRequest request, HomeworkQuizzes model)
        {

            var result = _historyAppService.GetAll().Include(x=> x.HomeworkQuiz).ThenInclude(x=> x.HomeworkQuizQuestions).Where(x => string.IsNullOrEmpty(x.DeleterUsername) && x.HomeworkQuizId == model.Id).OrderByDescending(x => x.Score).ToList();
        ...
        public async Task<ActionResult> DownloadScore(Guid id)
        {
            ...
            try
            {
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    var hwQuizScore = await _historyAppService.GetAll().Where(x=>x.HomeworkQuizId == id && string.IsNullOrEmpty(x.DeleterUsername)).ToListAsync();
                    ...
                    foreach (var score in hwQuizScore)
                    {
                        ...
                        //var details = _historyAnswerAppService.GetByHomeworkQuizHistory(score.Id);
