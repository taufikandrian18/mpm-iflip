...
{
    [AbpMvcAuthorize]
    public class InboxController : FLPControllerBase
    {
        ...
        public IActionResult Cascading_Get_Karesidenan([DataSourceRequest]DataSourceRequest request, string channel)
        {
            ...
            if (string.IsNullOrEmpty(channel))
            {
                ...
                var res = karesidenan.Select(c => new { karesidenan = c }).ToList();
                res.Add(new { karesidenan = " Semua Karesidenan" });
                result=res.OrderBy(x=> x.karesidenan).ToDataSourceResult(request);
            }
            else
            {
        ...
        {
            DataSourceResult result = new DataSourceResult();
            if (string.IsNullOrEmpty(karesidenan) && string.IsNullOrEmpty(channel) || karesidenan == " Semua Karesidenan")
            {
                DataSourceRequest req = new DataSourceRequest();
                var kota = _dealerAppService.GetKota().Select(x => new { kota = x }).ToList();
                kota.Add(new { kota = " Semua Kota" });
                result = kota.OrderBy(x => x.kota).ToDataSourceResult(req);
            }
            else 
            {
                ...
                else if (string.IsNullOrEmpty(channel))
                {
                    ...
                    kota.Add(new {karesidenan = karesidenan,  kota = " Semua Kota" });
                    result = kota.OrderBy(x => x.kota).ToDataSourceResult(request);
                }
                else
                {
        ...
        public IActionResult Cascading_Get_Dealer([DataSourceRequest]DataSourceRequest request, string channel, string karesidenan, string kota)
        {
            ...
            if (kota.IsNullOrWhiteSpace() || kota == " Semua Kota" && !channel.IsNullOrWhiteSpace())
            {
                DataSourceRequest req = new DataSourceRequest();
                var dealer = _dealerAppService.GetAll().Select(x => new { kota = x.Kota, dealer = x.Nama, kode = x.KodeDealerMPM }).ToList();
                DataSourceResult result = dealer.ToDataSourceResult(req);
                return Json(result);
            } else if (kota.IsNullOrWhiteSpace() || kota == " Semua Kota") {
                DataSourceRequest req = new DataSourceRequest();
                var dealer = _dealerAppService.GetAll().Where(x => x.Channel == channel).Select(x => new { kota = x.Kota, dealer = x.Nama, kode = x.KodeDealerMPM }).ToList();
                DataSourceResult result = dealer.ToDataSourceResult(req);
                return Json(result);
            }
            else {
                var dealer = _dealerAppService.GetAll().Where(x => x.Channel == channel && kota.Contains(x.Kota)).Select(x => new { kota = x.Kota, dealer = x.Nama, kode = x.KodeDealerMPM }).ToList();
        ...
        public IActionResult Get_User([DataSourceRequest]DataSourceRequest request, string channel, string karesidenan, string kota, string dealer,int limit, int page)
        {
         ...
         /*   if (limit == 0 )
            ...
            string resource = "";
            if (karesidenan == " Semua Karesidenan" && kota == " Semua Kota" && dealer.IsNullOrWhiteSpace()) {
                switch (resource)
                {
                    case "H1":
                        internalUser = _internalUserAppService.GetAll().Where(x => x.Channel == resource).ToList();
                        break;
                    case "H2":
                        internalUser = _internalUserAppService.GetAll().Where(x => x.Channel == resource).ToList();
                        break;
                    case "HC3":
                        ...
                        break;
                    default:
                        internalUser = _internalUserAppService.GetAll().ToList();
                        break;
                }
            } else if (karesidenan == " Semua Karesidenan" && kota == " Semua Kota" ) {
                switch (resource)
                {
                    case "H1":
                        internalUser = _internalUserAppService.GetAll().Where(x => x.Channel == resource).Where(x => x.DealerName.Contains(dealer)).ToList();
                        break;
                    case "H2":
                        internalUser = _internalUserAppService.GetAll().Where(x => x.Channel == resource).Where(x => x.DealerName.Contains(dealer)).ToList();
                        break;
                    case "HC3":
                        ...
                        break;
                    default:
                        internalUser = _internalUserAppService.GetAll().Where(x => x.DealerName.Contains(dealer)).ToList();

                        break;
                }

            }
            else if (roles.FirstOrDefault().Contains("H1"))
            {
                resource = "H1";
                ...
                {
                    internalUser = _internalUserAppService.GetAll().Where(x => x.Channel == resource && kota.Contains(x.DealerKota) && dealer.Contains(x.DealerName)).ToList();
                }
            }
            ...
            else if (roles.FirstOrDefault().Contains("HC3"))
            {
                ...
                {
                    internalUser = _internalUserAppService.GetAll().Where(x => x.Channel == channel && kota.Contains(x.DealerKota) && dealer.Contains(x.DealerName)).ToList();
                }
            }
            
            //return Json(internalUser.ToDataSourceResult(request));
            return Json(internalUser.OrderBy(x => x.IDGroupJabatan).ThenBy(x => x.IDMPM));
        }
