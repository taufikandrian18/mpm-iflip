...
{
    public class RoleplayController : FLPControllerBase
    {
        ...
        {
            RolePlays model = new RolePlays();
            return View(model);
        }

        [HttpPost]
        public async Task<IActionResult> Create(RolePlays model, string submit, IEnumerable<IFormFile> images)
        {
            if(model != null)
            {
        ...
        [HttpPost]
        public IActionResult Edit(RolePlays model, string submit)
        {
            if (model != null)
            {
                model.LastModifierUsername = this.User.Identity.Name;
        ...
        public IActionResult Grid_Destroy([DataSourceRequest]DataSourceRequest request, RolePlays item)
        {
        ...
        public IActionResult BackToDetail(RolePlayDetails item)
        {
            var model = _appService.GetById(item.RolePlayId);
            return View("Detail",model);
        }

        public IActionResult CreateDetail(Guid id)
        {
            RolePlayDetails model = new RolePlayDetails();
            model.RolePlayId = id;
        ...
        [HttpPost]
        public async Task<IActionResult> CreateDetail(RolePlayDetails model, string submit)
        {
            if (model != null)
            {
                model.Id = Guid.NewGuid();
            ...
            return Json(new { success = true, id = model.RolePlayId });
        }
        ...
        [HttpPost]
        public IActionResult EditDetail(RolePlayDetails model, string submit)
        {
            if (model != null)
            {
                model.LastModifierUsername = this.User.Identity.Name;
        ...
        public IActionResult Grid_Detail_Read([DataSourceRequest]DataSourceRequest request, RolePlays roleplay)
        {
            DataSourceResult result = _detailAppService.GetAll().Where(x => x.RolePlayId == roleplay.Id && string.IsNullOrEmpty(x.DeleterUsername)).OrderBy(x=>x.CreationTime).ToDataSourceResult(request);
        ...
        public IActionResult BackToResult(Guid id)
        {
            ...
            var model = _appService.GetById(result.RolePlayId);
            var dealer = _assignmentAppService.GetAll().SingleOrDefault(x => x.RolePlayId == model.Id && x.KodeDealerMPM == result.KodeDealerMPM);
            //return RedirectToAction("Result",model);
            //return Result(model.Id);
            return Result(dealer.Id);
        }
        //[DontWrapResult(WrapOnError = false, WrapOnSuccess = false)]
        //public ActionResult Grid_Result_Read([DataSourceRequest]DataSourceRequest request, RolePlays roleplay)
        //{
        //    var tmp = _resultAppService.GetAll().Where(x => x.RolePlayId == roleplay.Id && string.IsNullOrEmpty(x.DeleterUsername))
        ...
        [DontWrapResult(WrapOnError = false, WrapOnSuccess = false)]
        public ActionResult Grid_Result_Read([DataSourceRequest]DataSourceRequest request, AssignmentDealerVM roleplay)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> CalculatePoint([FromBody]List<ValidationVM> validations, Guid id)
        {
            double total = 0;
        ...
        [DontWrapResult(WrapOnError = false, WrapOnSuccess = false)]
        public ActionResult Grid_Validate_Read([DataSourceRequest]DataSourceRequest request, RolePlayResults rolePlayResults)
        {
            List<RoleplayResultDetailVM> details = new List<RoleplayResultDetailVM>();
            //var tmp = _detailAppService.GetAll().Where(x => x.RolePlayId == rolePlayResults.RolePlayId && string.IsNullOrEmpty(x.DeleterUsername)).OrderBy(x=>x.CreationTime)
            //        .Select(x => new RoleplayResultDetailVM
            //        {
            //            id = x.Id,
            //            title = x.Title,
            //            order = x.Order,
            //            rolePlayId = x.RolePlayId,
            ...
            foreach(var tmpresult in tmpResult)
            {
                ...
                //else
                //{
                //    detail = _detailAppService.GetAll().FirstOrDefault(x => x.RolePlayId == rolePlayResults.RolePlayId && !string.IsNullOrEmpty(x.DeleterUsername) && tmpresult.Title == x.Title);
                //    var tmpId = _resultAppService.GetById(tmpresult.RolePlayResultId).RolePlayId;
                //    details.Add(new RoleplayResultDetailVM
                //    {
                //        id = detail.Id,
                ...
                //        rolePlayId = tmpId,
                //        rolePlayResultId = tmpresult.RolePlayResultId
                //    });
                //}
            ...
            //{
            //    var detail = tmpResult.SingleOrDefault(x => x.RolePlayResultId == value.rolePlayResultId && x.Id == value.id);
        ...
        {
            RolePlayResults result = _resultAppService.GetById(Guid.Parse(id));
        ...
        {
            RolePlayResults result = _resultAppService.GetById(Guid.Parse(id));
            RolePlayResultDetails resultDetail = new RolePlayResultDetails();
            foreach (var item in pass)
            {
                RolePlayDetails details = _detailAppService.GetById(Guid.Parse(item));
                resultDetail = _detailResultAppService.GetAll().SingleOrDefault(x => x.Title == details.Title && x.Order == details.Order && x.RolePlayResultId == result.Id);
                ...
                {
                    resultDetail = new RolePlayResultDetails
                    {
                        Id = Guid.NewGuid(),
                        Title = details.Title,
                        Order = details.Order,
                        ...
                        DeleterUsername = "",
                        IsMandatorySilver = details.IsMandatorySilver,
                        IsMandatoryGold = details.IsMandatoryGold,
                        IsMandatoryPlatinum = details.IsMandatoryPlatinum,
                        AfterPassed = true,
                        AfterDismiss = false,
                        AfterNotPassed = false,
                        LastModificationTime = DateTime.UtcNow.AddHours(7),
                        LastModifierUsername = this.User.Identity.Name,
                        RolePlayResultId = Guid.Parse(id),

                    };
                    _detailResultAppService.Create(resultDetail);
                }
        ...
        public ActionResult DownloadAllExcel(Guid id)
        {
            ...
            var assignments = _assignmentAppService.GetAll().Where(x => x.RolePlayId == id);


            string rootFolder = _hostingEnvironment.WebRootPath;
            string excelName = "Roleplay " + selfRecordings.Title + ".xlsx";
            ...
            try
            {
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    foreach (var assigned in assignments)
                    {
                        ...
                        var details = selfRecordings.RolePlayResults.Where(x => x.KodeDealerMPM == assigned.KodeDealerMPM && x.RolePlayId == id);

                        var workSheetDetail = package.Workbook.Worksheets.Add("Roleplay " + assigned.KodeDealerMPM);
        ...
        public ActionResult DownloadDetail(RolePlays model)
        {
            model = _appService.GetById(model.Id);
            string rootFolder = _hostingEnvironment.WebRootPath;
            string excelName = "Roleplay "+ model.Title + " Detail.xlsx";

            var details = _detailAppService.GetAll().Where(x => x.RolePlayId == model.Id).ToList();
            ...
            try
            {
                using (var package = new ExcelPackage(stream))
                {
                    var workSheet = package.Workbook.Worksheets.Add("Roleplay Detail");
        ...
        public ActionResult DownloadTemplate(RolePlays model)
        {
            model = _appService.GetById(model.Id);
            string rootFolder = _hostingEnvironment.WebRootPath;
            string excelName = "Roleplay Detail Template.xlsx";
            ...
            try
            {
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    var workSheet = package.Workbook.Worksheets.Add("Roleplay Detail Template");
        ...
        [HttpPost]
        public async Task<IActionResult> ImportExcel(RolePlays model, string submit, IEnumerable<IFormFile> files)
        {
            if (model != null)
            {
                if (files.Count() > 0)
                {
                    ...
                    if (!Path.GetExtension(file.FileName).Equals(".xlsx", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        return Redirect("~/Roleplay/ImportExcel/" + model.Id);
                    }
                    ...
                    using (var stream = new MemoryStream())
                    {
                        ...
                        using (var package = new ExcelPackage(stream))
                        {
                            ExcelWorksheet worksheet = package.Workbook.Worksheets["Roleplay Detail Template"];
                            if (worksheet == null)
                            {
                            ...
                            for (int row = 2; row <= rowCount; row++)
                            {
                                ...
                                RolePlayDetails item = new RolePlayDetails
                                {
                                    Id = Guid.NewGuid(),
                                    Title = judul,
                                    Order = order,
            ...
            return Redirect("~/Roleplay/Detail/" + model.Id);
        }
        ...
        public ActionResult DownloadResults(string id, string title, string namaDealer)
        {
            ...
            string rootFolder = _hostingEnvironment.WebRootPath;
            string excelName = "Roleplay " + model.Title + " Result.xlsx";

            var details = _resultAppService.GetAll().Where(x => x.RolePlayId == model.Id && x.NamaDealerMPM == namaDealer).OrderBy(x=>x.IDMPM).ToList();
            ...
            try
            {
                using (var package = new ExcelPackage(stream))
                {
                    var workSheet = package.Workbook.Worksheets.Add("Roleplay Result");
        ...
        public async Task<IActionResult> Get_Dealer([DataSourceRequest]DataSourceRequest request, string id, string channel, string kota)
        {
            if (kota == null || kota == "")
            {
                ...
                {
                    var assignedUser = _assignmentAppService.GetAll().Where(x => x.RolePlayId == Guid.Parse(id)).Select(x => x.KodeDealerMPM).ToList();
        ...
        public async Task<IActionResult> GetCountAssignedDealer(Guid id)
        {
            ...
            var assignedUser = _assignmentAppService.GetAll().Where(x => x.RolePlayId == id).Select(x => x.KodeDealerMPM).ToList();
        ...
        public async Task<IActionResult> Get_Assigned_Dealer([DataSourceRequest] DataSourceRequest request, string id, string channel, string karesidenan, string kota, string dealer, string jabatan, int skip, int pageSize)
        {
            ...
            if (!id.IsNullOrWhiteSpace())
            {
                var assignedDealer = _assignmentAppService.GetAll().Where(x => x.RolePlayId == Guid.Parse(id)).Select(x => x.KodeDealerMPM).ToList();
        ...
        public ActionResult InsertAllDealer(Guid id)
        {
            ...
            if (assignedDealer.Count > 0)
            {
                ...
                //var listDeleted = assignedDealer.Where(p => !selectedDealer.Any(l => p.Id == l.id));
        ...
        {
            DataSourceResult result = _assignmentAppService.GetAll().Where(x => x.RolePlayId == roleplay.Id).OrderBy(x=>x.KodeDealerMPM).ToList().ToDataSourceResult(request);
