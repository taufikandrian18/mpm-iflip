...
{
    [AbpMvcAuthorize]
    public class ManagementPanduanLayananController : FLPControllerBase
    {
        ...
        public ManagementPanduanLayananController(UserManager userManager, GuideAppService appService, GuideCategoryAppService categoryAppService, GuideAttachmentAppService attachmentAppService, IActivityLogAppService activityLogAppService)
        {
        ...
        public async Task<IActionResult> Grid_Read([DataSourceRequest]DataSourceRequest request)
        {
            ...
            //DataSourceResult result = _appService.GetAll().Where(x => x.IsTechnicalGuide == false && (x.Resource == resource || resource == null) && string.IsNullOrEmpty(x.DeleterUsername)).OrderByDescending(x => x.CreationTime).ToDataSourceResult(request);

            var model = _appService.GetAll().Where(x => x.IsTechnicalGuide == false && (x.Resource == resource || resource == null) && string.IsNullOrEmpty(x.DeleterUsername)).OrderByDescending(x => x.CreationTime);
        ...
        public IActionResult Grid_Destroy([DataSourceRequest]DataSourceRequest request, SalesTalks item)
        {
            ...
            {
                _appService.SoftDelete(item.Id, this.User.Identity.Name, "Panduan Layanan");
            }
